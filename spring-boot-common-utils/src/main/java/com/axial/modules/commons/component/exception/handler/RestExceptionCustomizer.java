...
@Slf4j
@AllArgsConstructor
public class RestExceptionCustomizer extends ResponseEntityExceptionHandler {

    @Override
    protected ResponseEntity<Object> handleHttpRequestMethodNotSupported(
            HttpRequestMethodNotSupportedException ex, HttpHeaders headers, HttpStatusCode status, WebRequest request) {

        return RestExceptionUtils.createGenericResponseEntityForError(HttpStatus.METHOD_NOT_ALLOWED, ex, RestExceptionMessageCode.METHOD_NOT_ALLOWED_ERROR, ex.getMessage(), null);
    }
    ...
    protected ResponseEntity<Object> handleArgumentException(BindException ex, HttpHeaders headers, HttpStatusCode status, WebRequest request) {
        ...
        return RestExceptionUtils.createGenericResponseEntityForError(HttpStatus.PRECONDITION_FAILED, ex, RestExceptionMessageCode.DEFAULT_VALIDATION_ERROR, RestExceptionMessageCode.DEFAULT_VALIDATION_ERROR.getMessage(), messages);
    }
    ...
    private ResponseEntity<Object> handleServletRequestBindingExceptions(ServletRequestBindingException ex, HttpHeaders headers, HttpStatus status, WebRequest request) {
        ...
        final Message message = new Message(Severity.ERROR, String.valueOf(status.value()), null, clientMessage);
        final List<Message> messages = Arrays.asList(message);

        return RestExceptionUtils.createGenericResponseEntityForError(status, ex, String.valueOf(status.value()), null, RestExceptionMessageCode.PARAMETER_BINDING_ERROR.getMessage(), messages);
    }
